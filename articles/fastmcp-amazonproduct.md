...
Amazon Product API のラッパーを使って、mcp 様にツールとして呼び出せるようにラップしただけです。
...
ランキングなどの取得は他のモジュール使わないと難しそうだったので、一旦諦めました。
一個の mcp になんでも出来る様にするんじゃなくて、別の MCP を用意してリミックスして運用するというのがシンプルかなとか思いました。
